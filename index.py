...
class CodeEditor:
    def __init__(self, root):
        ...
        self.linenumbers = tk.Text(self.root, width=3, padx=5, takefocus=0, border=0, background='#2f3136', foreground='white', font=("Bahnschrift", 12), state='disabled')
        self.linenumbers.pack(side=tk.LEFT, fill=tk.Y)
        self.text_widget = tk.Text(self.root, font=("Bahnschrift", 12), bg='#2f3136', fg='white', insertbackground='white')
        self.text_widget.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.text_widget['yscrollcommand'], self.linenumbers['yscrollcommand'] = self.scroll_y, self.scroll_y
    ...
    def update_linenumbers(self, *args):
        ...
        if int(lines) > 999:
            messagebox.showerror('Error', 'Maximum number of lines exceeded. Why do you even need that much lines my guy?')
